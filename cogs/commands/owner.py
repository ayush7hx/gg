...
class Owner(commands.Cog):
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<:ztick:1448951767990796298> INF Staffs", description=f"\n{staff_display}", color=0xFF0000)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of INF [{len(self.client.guilds)}]",
            color=0xFF0000,
            per_page=10),
            ctx=ctx)
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("<:ztick:1448951767990796298> | **Successfully Restarting INF It Takes 10 seconds**")
        restart_program()
