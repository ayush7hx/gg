    ...
    async def invites(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**Left:** `{left}`\n"
                f"**Rejoins:** `{rejoin}`\n\n"
                f"{EMOJI_INVITE} **Get INF Premium Lifetime Join Support Here**"
            )
        )
        embed.set_thumbnail(url=member.avatar.url if member.avatar else member.default_avatar.url)
        embed.set_footer(text="Powered By Made by Ayush7hx", icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
