...
async def on_ready():
    await client.wait_until_ready()
    client.emoji_index = build_emoji_index(client)
...
async def on_guild_join(guild: discord.Guild):
    ...
    if log_channel:
        await log_channel.send(f"INF has been added to the server: **{guild.name}** (ID: `{guild.id}`)")
...
async def on_command_completion(context: commands.Context) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        embed.set_footer(text="Made by Ayush7hx", icon_url=client.user.display_avatar.url)
...
@app.route('/')
def home():
    return "Made by Ayush7hx"

def run():
    # Render supplies PORT for web services; retain a local fallback.
    port = int(os.getenv("PORT", "19346"))
